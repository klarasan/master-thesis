...
def fetch_variable(var, points, num_years):
    ...
    while(error and j < 10):
        try: 
            datasets = [nc.Dataset(base_url),nc.Dataset(base_url),nc.Dataset(base_url),nc.Dataset(base_url)]
            dataset = datasets[0]
            ...
            for point in tqdm(points, desc=f"Downloading point data for {var}"):
                ...
                def get_var(dataset, var, time_indices, lat_idx, lon_idx, id):
                    return  [id, dataset.variables[var][time_indices, lat_idx, lon_idx]]
                var_data = []
                with ThreadPoolExecutor(max_workers=4) as executor:
                    futures = []
                    
                    data_future1 = executor.submit(get_var,datasets[0], var, time_indices, lat_idx, lon_idx, 0)
                    futures.append(data_future1)
                    data_future2 = executor.submit(get_var,datasets[1], var, time_indices, lat_idx, lon_idx+1, 1)
                    futures.append(data_future2)
                    data_future3 = executor.submit(get_var,datasets[2], var, time_indices, lat_idx+1, lon_idx+1, 2)
                    futures.append(data_future3)
                    data_future4 = executor.submit(get_var,datasets[3], var, time_indices, lat_idx+1, lon_idx, 3)
                    futures.append(data_future4)
                    
                    for future in concurrent.futures.as_completed(futures):
                        var_data.append(future.result())

                var_data.sort(key=lambda d: d[0])
                var_data00 = var_data[0][1]
                var_data01 = var_data[1][1]
                var_data11 = var_data[2][1]
                var_data10 = var_data[3][1]
...
def save_to_csv(X, Y, num_years, path=""):
    ...
    csv_path = f'test_complete_dataset_{num_years}_years_bilinear_interp.csv'
    if path:
        csv_path = path
