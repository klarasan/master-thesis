...
def fetch_variable(var, points, num_years):
    all_data = []
    try:
        with open(f"json_with_outliers/{var}_prev_points.json", "r") as file:
    ...
    while(error and j < 5):
        try: 
            ...
            for point in tqdm(points, desc=f"Downloading point data for {var}"):
                ...
                def get_var(dataset, var, time_indices, lat_idx, lon_idx, event_year):
                    if previous_points.get((lat_idx, lon_idx, event_year)) is not None:
                        return previous_points[(lat_idx, lon_idx, event_year)]
                    else:
                        val = dataset.variables[var][time_indices, lat_idx, lon_idx]
                        previous_points[(lat_idx, lon_idx, event_year)] = val
                    return val

                var_data00 = get_var(dataset, var, time_indices, lat_idx, lon_idx, event_year).data # var_data[0][1]
                var_data01 = get_var(dataset, var, time_indices, lat_idx, lon_idx+1, event_year).data # var_data[1][1]
                var_data11 = get_var(dataset, var, time_indices, lat_idx+1, lon_idx+1, event_year).data # var_data[2][1]
                var_data10 = get_var(dataset, var, time_indices, lat_idx+1, lon_idx, event_year).data  # var_data[3][1]
...
def save_to_csv(X, Y, num_years, path=""):
    ...
    csv_path = f'{num_years}_years_bilinear_interp_w_outliers.csv'
    if path:
        csv_path = path
